...
class _NonLocalBlockND(nn.Module):
    ...
    def forward(self, x):
        ...
        batch_size = x.size(0)
        transform = len(x.shape) == 4
        length = x.shape[1]
        if transform:
            original_shape = x.shape
            x = x.view(batch_size, length, -1).permute(0, 2, 1)
        ...
        return z.permute(0, 2, 1).view(original_shape) if transform else z
...
class ProjectionRefinement(nn.Module):
    def __init__(self):
        super(ProjectionRefinement, self).__init__()
...
class LowPassFilter(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(1, y.shape[1]):
            y[:, i] = self.rate * x[:, i] + (1 - self.rate) * y[:, i - 1]
